...
type AuthenticateDeliveryResponse = Either<WrongCredentialsError,{accessToken:string}>
...
export class AuthenticateDeliveryUseCase {
  ...
  async execute({ cpf, password }: AuthenticateDeliveryRequest): Promise<AuthenticateDeliveryResponse> {
    ...
    const passwordIsValid = await this.hashComparer.compare(password, deliveryMan.password)
