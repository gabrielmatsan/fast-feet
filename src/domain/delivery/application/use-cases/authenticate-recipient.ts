...
type AuthenticateRecipientResponse = Either<WrongCredentialsError,{accessToken:string}>
...
export class AuthenticateRecipientUseCase {
  ...
  async execute({ cpf, password }: AuthenticateRecipientRequest): Promise<AuthenticateRecipientResponse> {
    ...
    const passwordIsValid = await this.hashComparer.compare(password, recipient.password)
